...
export default function WidgetCard({ widget }: { widget: any }) {
  return (
    <Link 
      ...
      className="group block bg-white rounded-[24px] border border-toast-brown/20 overflow-hidden shadow-sm hover:shadow-md hover:border-forest-green/30 transition-all flex flex-col h-full"
    >
      {/* 썸네일 영역 */}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between text-xs font-medium text-ink/50 pt-4 border-t border-toast-brown/10 mt-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1" title="복사수">
              📋 {widget.copy_count || 0}
            </span>
          </div>
        </div>
